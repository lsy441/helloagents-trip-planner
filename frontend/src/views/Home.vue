...
function handleChatFillForm(data: Record<string, any>) {
  if (data.city) formData.city = data.city
  if (data.transportation) formData.transportation = data.transportation
  if (data.accommodation) formData.accommodation = data.accommodation
  if (data.preferences && Array.isArray(data.preferences)) {
    formData.preferences = [...new Set([...formData.preferences, ...data.preferences])]
  }
  if (data.free_text_input) {
    formData.free_text_input = formData.free_text_input
      ? `${formData.free_text_input}；${data.free_text_input}`
      : data.free_text_input
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await generateTripPlanStream(requestData, (msg) => {
    ...
    if (response.success && response.data) {
      ...
      localStorage.setItem('tripPlan', JSON.stringify(response.data))
      // 保存原始表单数据(用于AI反馈调整)
      localStorage.setItem('tripFormData', JSON.stringify(requestData))
