  ...
  <div class="result-container">
    ...
    <div class="page-header">
      ...
      <a-space size="middle">
        ...
        <!-- AI反馈调整按钮 (LangGraph v2.0) -->
        <a-button 
          v-if="!editMode && tripPlan" 
          type="primary" 
            ...
            >
              ...
              <a-descriptions :column="1" bordered size="small">
                ...
                >
                  ...
                  <div v-if="meal.address" style="color: #666; font-size: 12px; margin-top: 4px;">
                    📍 {{ meal.address }}
                  </div>
                  <div v-if="meal.dishes && meal.dishes.length > 0" style="margin-top: 6px;">
                    <a-tag v-for="(dish, idx) in meal.dishes" :key="idx" color="orange" style="margin: 2px 4px 2px 0;">
                      {{ dish }}
                    </a-tag>
                  </div>
                  <div v-if="meal.estimated_cost" style="color: #1890ff; font-size: 12px; margin-top: 4px;">
    ...
    >
      <div class="feedback-content">
        <a-alert 
          message="智能调整说明" 
          description="告诉AI您想调整的部分,系统将只修改指定内容,其他部分保持不变。" 
          type="info" 
          show-icon 
        ...
        <a-form layout="vertical">
          <a-form-item label="调整目标">
            <a-select v-model:value="feedbackTarget" placeholder="选择要调整的部分">
              <a-select-option value="整体">整体计划</a-select-option>
              <a-select-option value="第1天">第1天行程</a-select-option>
              <a-select-option value="第2天">第2天行程</a-select-option>
              <a-select-option value="第3天">第3天行程</a-select-option>
              <a-select-option value="酒店">酒店推荐</a-select-option>
              <a-select-option value="餐饮">餐饮安排</a-select-option>
              <a-select-option value="景点">景点安排</a-select-option>
              <a-select-option value="交通">交通方案</a-select-option>
            </a-select>
          </a-form-item>
          
          <a-form-item label="调整需求">
            <a-textarea
              v-model:value="feedbackContent"
              :rows="4"
              placeholder="例如: 第2天景点太多,减少到2个; 换一家更便宜的酒店; 增加当地特色小吃推荐..."
              :maxlength="500"
              show-count
            />
          </a-form-item>
          ...
          <!-- 快捷调整选项 -->
          <a-form-item label="快捷调整">
            <a-space wrap>
              <a-tag 
                v-for="(item, index) in quickFeedbackOptions" 
                :key="index"
                :color="feedbackTarget === item.target ? 'blue' : 'default'"
                style="cursor: pointer; padding: 4px 12px;"
                @click="selectQuickFeedback(item)"
              >
                {{ item.label }}
              </a-tag>
            </a-space>
...
// AI反馈调整相关状态 (LangGraph v2.0)
const showFeedbackModal = ref(false)
const feedbackLoading = ref(false)
const feedbackTarget = ref('整体')
const feedbackContent = ref('')
const originalFormData = ref<any>(null)

// 快捷调整选项
const quickFeedbackOptions = [
  { label: '减少景点数量', target: '景点', content: '景点太多,每天减少到2-3个' },
  { label: '换更便宜的酒店', target: '酒店', content: '换一家性价比更高的酒店,预算减半' },
  { label: '增加美食推荐', target: '餐饮', content: '增加当地特色小吃和网红餐厅推荐' },
  { label: '优化交通路线', target: '交通', content: '优化景点之间的交通路线,减少路程时间' },
  { label: '增加户外活动', target: '整体', content: '增加更多户外活动和自然景观游览' },
  { label: '适合带小孩', target: '整体', content: '调整为适合亲子游的行程,增加儿童友好景点' }
]
...
async function handleChatAdjustPlan(data: { target: string; feedback: string }) {
  if (!originalFormData.value) {
    message.error('无法获取原始请求信息,请重新生成计划')
    return
  }
  ...
  try {
    const feedbackReq: FeedbackRequest = {
      original_request: originalFormData.value,
      feedback: data.feedback,
      target: data.target,
    }

    const response = await updatePlanWithFeedback(feedbackReq)
...
onMounted(async () => {
  ...
  if (data) {
    ...
    if (formData) {
      originalFormData.value = JSON.parse(formData)
    }
    await loadAttractionPhotos()
    await nextTick()
    initMap()
...
const saveChanges = () => {
  ...
  // 更新localStorage
  if (tripPlan.value) {
    localStorage.setItem('tripPlan', JSON.stringify(tripPlan.value))
  }
...
// ========== AI反馈调整功能 (LangGraph v2.0) ==========

// 选择快捷调整选项
const selectQuickFeedback = (item: { label: string; target: string; content: string }) => {
  feedbackTarget.value = item.target
...
const submitFeedback = async () => {
  ...
  try {
    ...
    console.log('📤 发送反馈调整请求:', feedbackReq)
    ...
    if (response.success && response.data) {
      ...
      message.success('✅ AI已根据您的反馈智能调整完成!')
    } else {
      throw new Error(response.message || '调整失败')
    }
